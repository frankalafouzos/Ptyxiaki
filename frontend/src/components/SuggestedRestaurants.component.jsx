...
const SuggestedRestaurants = () => {
    ...
    useEffect(() => {
        ...
        fetch("http://localhost:5000/restaurants/top-restaurants", { method: "GET" })
            .then((response) => {
    ...
    useEffect(() => {
        if (isUser && user) {
            fetch("http://localhost:5000/suggestions", {
                ...
                .then((response) => {
                    if (!response.ok) throw new Error("Failed to fetch restaurant recommendations");
                    return response.json();
                })
                .then((data) => {
                ...
                <>
                    {isUser && (
                        <>
                            ...
                            {loadingSuggestions ? (
                                <>
                                    <div className="loading-spinner">⏳ Finding the best options...</div>
                                    {topRestaurants.length > 0 && renderTopCarousel(topRestaurants)}
                                </>
