...
const AdminPendingEditDetail = () => {
  ...
  const handleApprove = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/admins/approve-edit/${id}`,
        {
          method: "POST",
          headers: {
  ...
  const handleReject = async () => {
    try {
      const response = await fetch(
        `${process.env.REACT_APP_API_URL}/admins/reject-edit/${id}`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          ...
          return (
            <div>
              <div className="text-danger">
                <small>
                  From: Tables 2: {value.old.tablesForTwo}, Tables 4:{" "}
                  {value.old.tablesForFour}, Tables 6: {value.old.tablesForSix},
                  Tables 8: {value.old.tablesForEight}
                </small>
              </div>
              <div className="text-success">
                <small>
                  To: Tables 2: {value.new.tablesForTwo}, Tables 4:{" "}
                  {value.new.tablesForFour}, Tables 6: {value.new.tablesForSix},
                  Tables 8: {value.new.tablesForEight}
                </small>
              </div>
            </div>
          );
          ...
          return (
            <div>
              <div className="text-danger">
                <small>
                  From:{" "}
                  {Array.isArray(value.old) ? value.old.join(", ") : value.old}
                </small>
              </div>
              <div className="text-success">
                <small>
                  To:{" "}
                  {Array.isArray(value.new) ? value.new.join(", ") : value.new}
                </small>
              </div>
            </div>
          );
