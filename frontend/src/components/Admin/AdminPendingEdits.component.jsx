  ...
  useEffect(() => {
    ...
    if (searchTerm) {
      ...
      result = result.filter(
        (edit) =>
          ...
          (edit.ownerDetails?.firstname || "")
            .toLowerCase()
            .includes(searchLower) ||
          (edit.ownerDetails?.lastname || "")
            .toLowerCase()
            .includes(searchLower)
      );
    }
    ...
    if (filters.changeTypes.length > 0) {
      result = result.filter((edit) => {
        ...
        return filters.changeTypes.some(
          (type) => edit.changes[type] !== undefined
        );
      });
    }
      ...
      {pendingEdits.length === 0 ? (
        <Card className="text-center p-5">
          <Card.Body>
            ...
            <Card.Text>
              There are currently no edit requests waiting for approval.
            </Card.Text>
          </Card.Body>
        </Card>
      ) : (
        <>
          ...
          <div className="mb-2 text-muted">
            {filteredEdits.length} result{filteredEdits.length !== 1 ? "s" : ""}{" "}
            found
          </div>
