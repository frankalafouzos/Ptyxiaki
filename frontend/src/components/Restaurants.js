                    ...
                    <div key={index} className="card m-5 d-flex justify-content-between" style={{ width: '40rem', borderRadius: '10%' }}>
                        ...
                        <div id={`Restaurant${index}`} className="carousel slide" data-ride="carousel">
                            <ol className="carousel-indicators">
                                {getImagesForRestaurant(restaurant.imageID).map((img, idx) => (
                            ...
                            <div className="carousel-inner">
                                {getImagesForRestaurant(restaurant.imageID).map((img, idx) => (
                                    <div key={idx} className={`carousel-item ${idx === 0 ? 'active' : ''}`}>
                                        <img style={{ width: 'auto', height: '30rem', borderRadius: '10% 10% 0% 0%' }} className="card-img-top d-block w-100 images" src={img.link} alt={`Slide ${idx}`} />
                                    </div>
                                ))}
                            </div>
