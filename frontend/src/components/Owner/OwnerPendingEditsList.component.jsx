  ...
  useEffect(() => {
    const getOwnerId = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5000/owners/ownerprofile`, {
          method: 'POST',
          headers: {
    ...
    const fetchPendingEdits = async (id) => {
      try {
        const response = await fetch(`http://localhost:5000/api/pending-edits/owner/${id}`);
          ...
          {pendingEdits.map((edit) => (
            <Col md={6} lg={4} className="mb-4" key={edit._id}>
              <Card className="h-100 pending-edit-card">
                ...
                <Card.Body>
                  <Card.Title>{edit.restaurant?.name || 'Unknown Restaurant'}</Card.Title>
                  <Card.Text>
                    <strong>Changes:</strong> {summarizeChanges(edit.changes)}
                  </Card.Text>
                </Card.Body>
                <Card.Footer>
                  <Link to={`/owner/pending-edits/${edit._id}`} className="btn btn-outline-primary w-100">
                    View Details
                  </Link>
                </Card.Footer>
              </Card>
