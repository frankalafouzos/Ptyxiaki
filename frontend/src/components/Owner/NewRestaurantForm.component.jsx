...
const detectChanges = (originalData, newData) => {
    ...
    const fieldsToCompare = [
        ...
        'email', 'description', 'Bookingduration', 'openHour', 'closeHour', 'closedDays'
    ];
        ...
        if (newData[field] !== undefined &&
            ...
            if (Array.isArray(newData[field])) {
                ...
                if (!originalData[field] ||
                    JSON.stringify(newData[field].sort()) !== JSON.stringify(originalData[field].sort())) {
                    changes[field] = {
...
const submitRestaurantEdit = async (restaurantId, formData, originalData, uploadedImages, deletedImages, owner, capacityChanges, closedDaysChanges) => {
    try {
        ...
        if (imageChanges || hasImageChanges) {
            changes.images_changes = imageChanges || {
                ...
                deleted: deletedImages.length > 0 ? deletedImages.map(img => ({
...
const RestaurantForm = ({ restaurantData, capacities, DefaultClosedDays, handleSubmit, ownerId, screenType }) => {
    ...
    const onSubmitAdd = async (e) => {
        ...
        try {
            ...
            if (restaurantId) {
                await fetch("http://localhost:5000/restaurants/deleteAll/" + restaurantId, {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json',
                    },
    ...
    const onSubmitEdit = async (e) => {
        ...
        try {
            ...
            } else if ((originalClosedDays?.length || 0) !== (newClosedDays?.length || 0)) {
                // Different lengths - definitely changed
                hasClosedDaysChanges = true;
            } else {
                // Same length - ensure string comparison
                const sortedOriginal = [...(originalClosedDays || [])].sort().map(String);
                const sortedNew = [...(newClosedDays || [])].sort().map(String);
                hasClosedDaysChanges = JSON.stringify(sortedOriginal) !== JSON.stringify(sortedNew);
            }

            // Add extra debugging to see exact values being compared
            console.log("Sorted Original:", JSON.stringify([...(originalClosedDays || [])].sort()));
            console.log("Sorted New:", JSON.stringify([...(newClosedDays || [])].sort()));
