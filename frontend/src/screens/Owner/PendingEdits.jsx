// Example component to display pending edits for restaurant owners
...
const PendingEditsList = ({ ownerId }) => {
  ...
  useEffect(() => {
    const fetchPendingEdits = async () => {
      try {
        const response = await fetch(`http://localhost:5000/pendingEdits/owner/${ownerId}`);
        if (response.ok) {
            ...
            <Card.Body>
              ...
              <ul>
                {Object.entries(edit.changes).map(([field, value]) => (
                  <li key={field}>
                    <strong>{field}:</strong> {field === 'images_changes' ? (
              ...
              <small>Submitted: {new Date(edit.submittedAt).toLocaleString()}</small>
            </Card.Body>
          </Card>
        ))
      )}
