...
import { FaSearch, FaTrash, FaUserShield, FaUserAltSlash } from 'react-icons/fa';
...
const AdminUsers = () => {
    ...
    const fetchUsers = async () => {
        try {
            ...
            const response = await fetch(`http://localhost:5000/admins/allUsers?${queryParams}`);
    ...
    const handleDeleteUser = async (userId) => {
        try {
            const response = await fetch(`http://localhost:5000/admins/user/${userId}`, {
                method: 'DELETE',
            });
            if (response.ok) {
    ...
    const handleToggleAdmin = async (userId, isAdmin) => {
        try {
            const response = await fetch(`http://localhost:5000/admins/user/${userId}/toggleAdmin`, {
                method: 'PATCH',
            });
            const data = await response.json();
            ...
            if (response.ok) {
                ...
                const updatedUsers = users.map((user) =>
                    user._id === userId ? { ...user, role: isAdmin ? 'User' : 'Admin' } : user
                );
                    ...
                    {users.length === 0 ? (
                        ...
                        users.map((user, index) => (
                            <tr key={user._id}>
                                ...
                                <td>{user.role}</td>
                                <td>
                                    <Button variant="danger" className="action-btn delete" onClick={() => handleDeleteUser(user._id)}>
                                        <FaTrash /> Delete
                                    </Button>
                                    {user.role !== 'Owner' && (
                                        <Button
